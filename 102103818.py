...
import zipfile
import urllib.error
# Assuming you have a custom function for sending emails
...
def search_youtube_videos(query, num_results):
...
def download_video(singer, n, start_time=20):
    count = 0
    query = singer + ' music video'
    results = search_youtube_videos(query, n + 20)
    for url in results:
        try:
            yt = YouTube(url)
            video = yt.streams.filter(file_extension='mp4').first()

            # Set the start time for the download
            video_url = video.url + f"&start={start_time}"
            ...
            for attempt in range(3):
                try:
                    ...
                    print(
                        f"Error downloading video (attempt {attempt+1}): {e}")
            ...
            pass  # Handle exceptions for unavailable videos or HTTP errors
...
with st.form("my_form"):
    ...
    if submitted:
        if not singer.strip():
            ...
            st.success(
                'Your mashup is ready! Check your email for the download link.')
